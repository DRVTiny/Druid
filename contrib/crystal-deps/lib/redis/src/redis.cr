...
class Redis
  DFLT_REDIS_DATABASE = 0_u16
  DFLT_REDIS_PORT = 6379
...
#  getter database : UInt16
  ...
  def initialize(@host = "localhost", @port = 6379, @unixsocket : String? = nil, @password : String? = nil,
                 @database = DFLT_REDIS_DATABASE, url = nil, @ssl = false, @ssl_context : OpenSSL::SSL::Context::Client? = nil,
                 @dns_timeout : Time::Span? = nil, @connect_timeout : Time::Span? = nil, @reconnect = true, @command_timeout : Time::Span? = nil)
    if url
      uri = URI.parse url
      @host = uri.host.to_s
      @port = uri.port || DFLT_REDIS_PORT
      @password = uri.password
      path = uri.path
      @database = path[1..-1].to_u16 if path && path.size > 1
      if @database > 0
  ...
  def database=(database_number : Number)
    ...
    redis_dbn = database_number.is_a?(UInt16) ? database_number : database_number.to_u16
#    puts "SELECT: attempt to -> #{redis_dbn}"
    res = string_command(["SELECT", redis_dbn.to_s])
    if res == "OK"
      @database = redis_dbn
#      puts "SELECT: switched to #{@database}"
    else
